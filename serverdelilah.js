...
const contraseñaJwt = "MafcxI9nlw";
...
function validarUsuario(req, res, next) {
  ...
  if (typeof nombreusuario != undefined && typeof contraseña != undefined) {
    sequelize
      ...
      .then(resultados => {
        ...
        if (usuario == undefined) {
          ...
          if (hashing.verify(contraseña, usuario.contraseña)) {
            usuario_token = {
              id_usuario: usuario.id,
              nombreusuario: nombreusuario,
              admin: usuario.admin
            };
            const token = jwt.sign({ usuario_token }, contraseñaJwt);
            req.locals = token;
            return next();
          } else {
...
function validarAdmin(req, res, next) {
  ...
  if (!token) {
    ...
    const verificado = jwt.verify(token, contraseñaJwt);
...
app.post("/usuarios", (req, res) => {
  ...
  contraseñaEncriptada = hashing.generate(contraseña);
    ...
    sequelize
      ...
      .then(resultados => {
        if (resultados.length == 0) {
          sequelize
            .query(
              "INSERT INTO usuarios (nombre_usuario, contraseña, nombre, apellido, mail, telefono,admin) VALUES (?,?,?,?,?,?,?)",
              {
                replacements: [
                  nombreusuario,
